...
class LowQualityFilterJudge(Judge):
    ...
    def __init__(self, config: JudgeConfig, all_turns: bool = False, measure_dir: str = ""):
    ...
    def format_conversation(self, row: dict) -> str | None:
        ...
        if len(conversation) < 2:
            return None
        if self.all_turns:
            parts = [f"{t['role'].upper()}: {t['content']}" for t in conversation]
            return "\n".join(parts)
        user_msg = conversation[0].get("content", "")
        asst_msg = conversation[1].get("content", "")
        if len(user_msg or "") + len(asst_msg or "") > 24000:
            return None
    ...
    def add_args(cls, parser: argparse.ArgumentParser):
        ...
        parser.add_argument("--all_turns", action="store_true",
                            help="Send full conversation instead of first turn only")
    ...
    def from_args(cls, args: argparse.Namespace) -> "LowQualityFilterJudge":
        ...
        return cls(config, all_turns=args.all_turns, measure_dir=args.measure_dir)
