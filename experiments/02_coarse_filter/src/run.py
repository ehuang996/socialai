...
Usage:
    uv run python src/run.py --measure anthropomorphism --stage coarse_filter [judge args...]
    uv run python src/run.py --measure anthropomorphism --stage low_quality_filter [judge args...]
    uv run python src/run.py --measure anthropomorphism --stage high_quality_filter [args...]
...
def main():
    ...
    if our_args.experiment_dir is not None and should_snapshot:
        src_dir = Path(__file__).parent  # i.e., src/
